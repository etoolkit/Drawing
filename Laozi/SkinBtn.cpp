...
//				SetTimer(2,20,NULL);
	...
	//			memDC.TextOut(m_textPos.x,m_textPos.y,str);
		...
		else
		{
			...
			posText.y = (rect.Height() - 20)/2;
			SIZE size1;
			GetTextExtentPoint32(memDC,str,str.GetLength(),&size1);
			posText.x = (rect.right - rect.left - size1.cx)/2;
			posText.y = (rect.bottom - rect.top - size1.cy)/2;
...
void CSkinBtn::SetImage(CString strNormal, CString strHover, CString strDown, CString strDisable,int x, int y)
{
